...
%{
    ...
    public TokenSource(InputStream stream, String source) throws IOException {
        this(new InputStreamReader(stream, "UTF-8"));
        this.source = source;
    }

    public TokenSource(URL source) throws IOException {
    ...
    private Token token(Terminal type) {
        return new Token(type, yytext());
    }

    private Token token(Terminal type, Object value) {
...
<YYINITIAL> {
    ...
    [Cc] "'" {Character} "'"
                    { detectSelector(); return token(CHARACTER, unescapeJava(yytext()).charAt(3)); }
    [Cc] "'" {OctalEscape} "'"
                    {
                        String text = yytext().substring(2, yylength() - 2);
                        detectSelector();
                        return token(CHARACTER, unescapeJava(octalToHex(text)).charAt(1));
                    }
    ...
    {Identifier}    { detectSelector(); return token(ID); }
    ...
    "<<"            { detectNewLine(); return token(SHIFT_LEFT); }
    "<"             { detectNewLine(); return token(LESS_THAN); }
    ">="            { detectNewLine(); return token(GREATER_THAN_EQUALS); }
    ">>"            { detectNewLine(); return token(SHIFT_RIGHT); }
    ">>>"           { detectNewLine(); return token(USHIFT_RIGHT); }
    ...
    {Double}        { return token(DOUBLE, Double.parseDouble(yytext())); }
...
<REGEX_OPTIONS_STATE> {
    [misx]+         { leaveState(); detectSelector(); return token(REGEX_OPTIONS, yytext()); }
    .               { leaveState(); yypushback(1); detectSelector(); return token(REGEX_OPTIONS, ""); }
}
